...
Ravencoin.prototype.getAPIMethods = function() {
  var methods = [
    ...
    ['getMNListFull', this, this.getMNList, 1],
...
Ravencoin.prototype._loadSpawnConfiguration = function(node) {
  ...
  var configPath = (spawnOptions['configPath']) ? path.resolve(spawnOptions.configPath) : path.resolve(spawnOptions.datadir, './ravendark.conf');
...
Ravencoin.prototype._getNetworkConfigPath = function() {
  var networkPath;
  if (this.node.network === ravencore.Networks.testnet) {
    networkPath = 'testnet3/raven.conf';
...
Ravencoin.prototype.getMNListFull = function(callback) {
  var self = this;
  self.client.masternodelist('full', function(err, response) {
    if (err) {
      callback(self._wrapRPCError(err), null);
    } else {
...
Ravencoin.prototype.getMNList = function(callback) {
  var self = this;
  var rawResults= {};
  var MNList = [];
  ...
  var checkSync = function checkSync(next) {
    self.isSynced(function (err, isSynced) {
      // removing this for now, bug in sync percentage means it never says fully synced
      // if (err) {
      //   return next(err);
      // }
      // if (!isSynced) {
